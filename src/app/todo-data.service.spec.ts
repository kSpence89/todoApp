...
describe('#deleteTodoById(id)', () => {
  ...
  it('should not be removing anything if todo with corresponding id is not found', inject([TodoDataService], (service: TodoDataService) => {
    let todo1 = new Todo({title: 'Hello 1', complete: false});
    let todo2 = new Todo({title: 'Hello 2', complete: true});
    service.addTodo(todo1);
    service.addTodo(todo2);
...
describe('#updateTodoById(id, values)',() => {
  ...
  it('should return todo with the corresponding id and updated data', inject([TodoDataService], (service: TodoDataService) => {
    ...
    let updateTodoById = service.updateTodoById(1, {
      title: 'new title'
    });
    expect(updateTodoById.title).toEqual('new title');
  }));
